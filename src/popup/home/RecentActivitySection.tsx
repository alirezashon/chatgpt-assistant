...
import { CAPABILITY_ACTIONS } from './home-actions';
  ...
  return (
    <section className="mt-[var(--ds-space-3)]">
      <div className="mb-[var(--ds-space-2)]">
        <SectionTitle icon={Clock3} title={recent.length === 0 ? 'Common goals' : 'Recent work'} />
      </div>
      <Panel className="grid gap-[var(--ds-space-1)] p-[var(--ds-space-2)]">
        {recent.length === 0
          ? CAPABILITY_ACTIONS.slice(0, 4).map((action) => (
              <CompactActionRow
