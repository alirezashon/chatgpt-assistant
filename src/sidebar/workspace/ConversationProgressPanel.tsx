...
export function ConversationProgressPanel({ task }: { readonly task: SidebarTask }) {
  return (
    <Panel className="p-[var(--ds-space-3)]">
      <SectionTitle icon={MessageSquareText} title="Conversation / progress" />
...
function TaskMessage({ message }: { readonly message: SidebarTaskMessage }) {
  ...
  return (
    <div className="grid grid-cols-[auto_1fr] gap-[var(--ds-space-2)] rounded-[var(--ds-radius-lg)] border border-[color:var(--ds-color-border)] bg-[var(--ds-color-surface-subtle)] p-[var(--ds-space-3)]">
      <Icon className={`mt-0.5 h-[var(--ds-icon-md)] w-[var(--ds-icon-md)] ${stageColor(message.kind)}`} />
